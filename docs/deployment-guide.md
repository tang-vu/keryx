...
- **Reasoning-provider watchdog** — `npm run check-llm` asks every credentialed model one real `decompose` question through the same engine the agent uses, and alerts when the default tier cannot answer. This exists because the fallback chain is deliberately unkillable: when DeepSeek retired the `deepseek-chat` wire name, every reasoning step dropped to the offline heuristic and production kept answering and settling real USDC with nothing surfacing it. A default-tier failure alerts loudly (every other pick falls back onto it); a broken alternative pick is reported without the alarm. `npm run deploy` installs it as an hourly cron (`# keryx-llm`, minute :15), logging to `data/backups/llm.log`. Fix by checking the wire name against the provider's own model list, then setting `KERYX_LLM_MODEL` / `KERYX_SYNTHESIS_MODEL`.
