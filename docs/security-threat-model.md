...
|----|-------------|-------------|------|
| R1 | **payTo redirect under bundle compromise, and for pre-registry sources** | Narrowed on 2026-07-09. Citation and fetch payTo are now validated against the on-chain registry on both sides (S11, S11b). Two gaps remain. (a) An attacker who can replace the served JavaScript can strip the browser's check; client-side validation is structurally incapable of surviving that, and the funded cap ($0.05–$1.00) is the ceiling. (b) 4 of 21 sources predate the registry and have no on-chain record; their payTo falls back to the mutable database, contained by the publicly enumerable payout wallet. | Register the remaining 4 sources on-chain. Bundle integrity (SRI / pinned frontend) is the only real answer to (a) and is out of scope for a single-origin app. |
...
- Register the 4 remaining pre-registry sources on-chain so no citation payTo falls back to the database (closes R1(b)).
