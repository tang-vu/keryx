...
|----|-------------|-------------|------|
| R1 | **payTo redirect under bundle compromise** | Narrowed on 2026-07-09, narrowed again on 2026-07-10. Citation and fetch payTo are validated against the on-chain registry on both sides (S11, S11b), and 19 of 20 sources now carry a registry record — new registrations write to the chain from the creator's own wallet, so the gap does not reopen. (a) An attacker who can replace the served JavaScript can strip the browser's check; client-side validation is structurally incapable of surviving that, and the funded cap ($0.05–$1.00) is the ceiling. (b) One source (`conzit-labs`) predates the registry and is creator-owned, so only its owner can register it; until then its payTo falls back to the database, contained by its publicly enumerable payout wallet. | Bundle integrity (SRI / pinned frontend) is the only real answer to (a) and is out of scope for a single-origin app. For (b), the owner registers it from their own wallet. |
...
- A "register on-chain" action for a creator-owned source listed before the registry was switched on (closes R1(b) for `conzit-labs`).
