...
/**
 * SourceRegistry V2 candidate — revision-checked catalog; not deployed.
 *
...
contract SourceRegistryV2 {
    ...
    modifier currentRevision(bytes32 id, uint64 expected) {
        ...
        // All mutations are local storage writes. A later validation failure rolls
        // this increment back together with the transaction.
        revisions[id] = expected + 1;
        _;
    }

    function registryVersion() external pure returns (uint8) { return 2; }
    ...
    /** Changes only price, and rejects every edit based on an obsolete revision. */
    ...
    /** One eth_call observes record and revision from the same state. */
    ...
    function _validateSplit(AuthorSplit[] calldata authors) private pure {
        ...
        // At least one author is required; total basis points must be exactly 10_000.
