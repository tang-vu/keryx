...
export function privateWorkerBootstrap(db: KeryxDB) {
  const env = process.env;
  if (env.KERYX_PRIVATE_WORKER_ENABLED === undefined || env.KERYX_PRIVATE_WORKER_ENABLED === "0") return null;
  try {
    if (env.KERYX_PRIVATE_WORKER_ENABLED !== "1" || env.KERYX_PRIVATE_RESEARCH_ENABLED !== "1"
    ...
    return createPrivateWorker(db, { signerAddress: account.address, signer: new BatchEvmScheme(account),
