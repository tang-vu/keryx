...
async function inspectFile(path: string) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.uid !== process.getuid!()
    || stat.nlink !== 1 || (stat.mode & 0o077) !== 0) throw new Error();
  return stat;
}

/** Existing owner-only application database, never created by queue recovery. */
export async function inspectWithdrawalApplicationDatabase(path: string) {
  try {
    if (!isAbsolute(path) || resolve(path) !== path) throw new Error();
    await inspectDirectory(dirname(path));
    const stat = await inspectFile(path);
    if (stat.size < 1) throw new Error();
    for (const suffix of ["-journal", "-wal", "-shm"]) {
      try { await inspectFile(path + suffix); }
      catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
    }
    return { device: stat.dev, inode: stat.ino };
  } catch { throw new Error("Protected withdrawal application database unavailable"); }
}
...
export async function inspectWithdrawalRelayFiles(directory: string) {
  try {
    const root = await inspectDirectory(directory);
    const policyPath = join(root, "policy.json"), databasePath = join(root, "mint.sqlite");
    const policyStat = await inspectFile(policyPath), databaseStat = await inspectFile(databasePath);
    if (policyStat.size < 1 || policyStat.size > 4096 || databaseStat.size < 1) throw new Error();
    for (const suffix of ["-journal", "-wal", "-shm"]) {
      try { await inspectFile(databasePath + suffix); }
      catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
    }
