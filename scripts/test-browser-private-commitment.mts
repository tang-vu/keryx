...
try {
  ...
  const result = await page.evaluate(`(async () => {
    ...
    const first=await window.commitment.createPrivateAuthorization(request,requirement,payer,payee,1788912000000);
    const second=await window.commitment.createPrivateAuthorization(request,requirement,payer,payee,1788912000000);
    return {nonce,fresh:first.salt!==second.salt&&first.authorization.nonce!==second.authorization.nonce,
  ...
  assert.equal(result.fresh, true); assert.equal(result.valid, true); assert.equal(result.changed, false); assert.equal(requests, 1);
  console.log("PASS: Chromium and Node commitment vector agrees; secure randomness and tamper rejection verified. No wallet, signing or payment requests.");
