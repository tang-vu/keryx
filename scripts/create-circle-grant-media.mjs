...
try {
  ...
  const scenes = [
    ...
    {
      id: "07-proof",
      narration: `At this production snapshot, Keryx has recorded ${m.totalQueries.toLocaleString("en-US")} questions and settled ${m.totalPayments.toLocaleString("en-US")} payments worth ${m.totalVolumeUsdc.toFixed(6)} in Arc testnet USDC, with ${m.totalCreatorPayoutsUsdc.toFixed(6)} paid to creator wallets. Registry records match Arc with zero mismatches, and creator withdrawals resolve to individual ArcScan transactions. Simulated and pending payments never enter the settled totals.`,
    },
    {
      id: "08-roadmap",
