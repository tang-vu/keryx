...
try {
  ...
  await context.route("**/*", async route => {
    ...
    if (url.pathname === "/api/session/credit") {
      await route.fulfill({ json: { status: "known", address: account.address, network: "eip155:5042002", available: "50000" } }); return;
    }
  ...
  assert.equal(signed, 1); assert.equal(paid, 1); assert.deepEqual(errors, []);
  console.log("PASS: React checkout review, changed question refusal, durable recovery download, one signature/POST, HTTP-500 uncertainty, reload/import GET-only recovery, verified completed receipt, substituted receipt refusal and explicit local deletion. Synthetic wallet; all HTTP intercepted.");
