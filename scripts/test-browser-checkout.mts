...
try {
  ...
  await page.getByText("No order was found.", { exact: false }).waitFor();
  await page.getByText("Saved data reports a seller payment acknowledgement", { exact: false }).waitFor();
  assert.equal(signed, 1); assert.equal(paid, 1);
  const exportDownload = page.waitForEvent("download");
  await page.getByRole("button", { name: "Export recovery file", exact: true }).click();
  const portableRecovery = await readFile((await (await exportDownload).path())!, "utf8");
  const portable = JSON.parse(portableRecovery);
  assert.equal(portable.schema, "keryx-buyer-recovery-v1");
  assert.deepEqual(portable.intent, intent);
  assert.equal(portable.acknowledgement.evidence.success, true);
  assert(!portableRecovery.includes('"signature"'));
  ...
  await page.getByLabel("Import recovery file", { exact: true }).setInputFiles({ name: "recovery.json", mimeType: "application/json", buffer: Buffer.from(portableRecovery) });
  await page.getByText("No order was found.", { exact: false }).waitFor();
  await page.getByText("Saved data reports a seller payment acknowledgement", { exact: false }).waitFor();
