...
declare global { interface Window { fundingJournal: typeof import("../lib/buyer/funding-journal"); } }
...
const bundle = await build({ stdin: { contents: `
  ...
  createRoot(document.getElementById('root')).render(React.createElement(React.StrictMode,null,
    React.createElement(ResearchFunding,{payer:'${payer}',initialAmount:0.05,disabled:false,onBusy:noop,onChanged:noop})));
...
try {
  ...
  console.log("PASS: Chromium funding replacement recovery, exact approval speedup, unknown hash stays locked, changed deposit call is not credited, and reload preserves history. Synthetic RPC; no signing or settlement.");
