...
ssh "$SSH" "cd $APP_DIR && node scripts/dependency-state.mjs invalidate && npm ci --no-audit --no-fund && node scripts/dependency-state.mjs record && NODE_OPTIONS=--max-old-space-size=1536 npm run typecheck && NODE_OPTIONS=--max-old-space-size=1536 npm run build"
